...
impl<'a> Definition<'a> {
    ...
    fn build_from_unicodes(input: &[u16]) -> Vec<Definition> {
        ...
        while idx < input.len() {
            let code = &input[idx];
            let may_be_kinzi_sequence: bool = idx + 2 < input.len() && input[idx + 1] == 0x103A && input[idx + 2] == 0x1039;
            clusters.push(
                Definition::new(MType::from_u16(code, may_be_kinzi_sequence), code)
            );
            if may_be_kinzi_sequence { idx += 3; }
            else { idx += 1; }
        }
        ...
        while idx < self.defs.len() {
            match self.defs[idx].m_type {
                ...
                MType::MR => {
                    ...
                    while base_c_idx + 1 < self.defs.len() && self.defs[base_c_idx].m_type != MType::C { base_c_idx += 1; }
                    if base_c_idx != idx {
                ...
                MType::A => {
                    ...
                    while prev_idx - 1 > 0 && self.defs[prev_idx - 1].m_type == MType::VBlw { prev_idx -= 1; }
                    if prev_idx != idx { self.defs.swap(prev_idx, idx); }
                }
                _ => {},
            }
            idx += 1;
...
mod tests {
    ...
    fn myanmar_complex_test() {
        ...
        let expected: &[u16] = &[4145, 4156, 4096, 4100, 4154, 4153, 4153, 4096, 4155, 4157, 4158, 4141, 4143, 4151, 4154, 4140, 4158, 4142, 4151, 4196, 4146, 4150, 4151, 4152, 4237];
