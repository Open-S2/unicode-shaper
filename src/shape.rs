...
pub fn shape_unicode(
    ...
    if (options & U_SHAPE_AGGREGATE_TASHKEEL_MASK) != 0 {
        ...
        let mut j: usize = 2 * source.len();
        if logical_order { j = 0; }
        let mut i: usize = source.len();
        if logical_order { i = 0; }
        let mut end: usize = source.len() - 1;
        if logical_order { end = source.len(); }
        ...
        while i != end {
            ...
            if aggregate_tashkeel && ((prev_link | curr_link) & COMBINE) == COMBINE && aggregation_possible {
                ...
                if prev < source[i] {
                    ...
                    tempsource[j] = source[i] - 0x064C + 0xFC5E;
                }
    ...
    match options & U_SHAPE_LETTERS_MASK {
        U_SHAPE_LETTERS_SHAPE => {
                ...
                ((options & U_SHAPE_TASHKEEL_MASK) != U_SHAPE_TASHKEEL_REPLACE_BY_TATWEEL)
            {
                ...
                shape_arabic(&mut output, 2); // , allocator);
            } else {
                // default Call the shaping function with tashkeel flag == 1
                shape_arabic(&mut output, 1); // , allocator);
        ...
        U_SHAPE_LETTERS_SHAPE_TASHKEEL_ISOLATED => {
            ...
            shape_arabic(&mut output, 0); // , allocator);
        },
        _ => {
    ...
    // copy a slice to a new slice "final_output" of output_size
    // and run through output, skip every LAMALEF_SPACE_SUB and TASHKEEL_SPACE_SUB
    let mut final_output = Vec::<u16>::with_capacity(output_size);
    for ch in output {
    ...
    if (options & U_SHAPE_DIRECTION_OUTPUT_BIDI) != 0 {
        return process_bidi_text(&final_output);
    }
...
mod tests {
    ...
    //     let binding_input: Vec<u16> = input.encode_utf16().collect();
    //     let input_utf16: &[u16] = binding_input.as_slice();
    //     let binding_expected: Vec<u16> = expected.encode_utf16().collect();
    //     let expected_utf16: &[u16] = binding_expected.as_slice();

    //     let result: &[u16] = &shape_unicode(input_utf16, &DEFAULT_OPTIONS);
    //     assert_eq!(result, expected_utf16);
    // }
