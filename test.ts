...
import { WasmTextShaper } from './src';
...
console.info(str.split('').map((n) => n.charCodeAt(0)));
const res = wasm.shapeString(str);
console.info(res);
console.info(res.split('').map((n) => n.charCodeAt(0)));
const adjust = adjustMedials(str.split(''));
console.info(adjust.join(''));
console.info(adjust.map((n) => n.charCodeAt(0)));

/**
 * Adjust the medials and vowels
 * @param fieldCodes - Array of field codes
 * @returns - Adjusted field codes
 */
